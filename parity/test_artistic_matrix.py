...
DEFAULTS = {"extrude": {"type": "blocks", "depthSource": "luminance"}, "halftone": {"mode": "color", "pattern": "dot"}, "morphology": {"mode": "dilate", "shape": "square"}, "wind": {"method": "blast", "direction": "fromLeft"}, "dither": {"type": "bayer4x4"}}
CHOICES = {
    ("extrude", "type"): {"0": "blocks", "1": "pyramids"}, ("extrude", "depthSource"): {"0": "luminance", "1": "random"},
    ("halftone", "mode"): {"0": "color", "1": "mono"}, ("halftone", "pattern"): {"0": "dot", "1": "line", "2": "circle"},
    ("morphology", "mode"): {"0": "dilate", "1": "erode"}, ("morphology", "shape"): {"0": "square", "1": "round"},
    ("wind", "method"): {"0": "wind", "1": "blast", "2": "stagger"}, ("wind", "direction"): {"0": "fromLeft", "1": "fromRight"},
    ("dither", "type"): {"0": "bayer2x2", "1": "bayer4x4", "2": "bayer8x8", "3": "dot", "4": "line", "5": "crosshatch", "6": "noise", "7": "errorDiffusion"},
...
def fixture_values(effect, params):
    found = set(); pattern = re.compile(rf"\.{effect}\(([^()]*)\)")
    for path in PROGRAMS.glob("*.dsl"):
        ...
        for item in match.group(1).split(","):
            if ":" in item:
                key, value = (part.strip() for part in item.split(":", 1)); args[key] = CHOICES.get((effect, key), {}).get(value, value)
...
class ArtisticMatrixTests(unittest.TestCase):
    ...
    def assert_matrix(self, effect, params, expected):
        actual = fixture_values(effect, params); self.assertTrue(expected <= actual, f"{effect} missing {sorted(expected - actual)}")
    def test_extrude_type_by_depth_source(self): self.assert_matrix("extrude", ("type", "depthSource"), {(t, d) for t in ("blocks", "pyramids") for d in ("luminance", "random")})
    def test_halftone_live_mode_by_pattern(self): self.assert_matrix("halftone", ("mode", "pattern"), {("color", "dot"), ("mono", "dot"), ("mono", "line"), ("mono", "circle")})
    def test_morphology_mode_by_shape(self): self.assert_matrix("morphology", ("mode", "shape"), {(m, s) for m in ("dilate", "erode") for s in ("square", "round")})
    def test_wind_method_by_direction(self): self.assert_matrix("wind", ("method", "direction"), {(m, d) for m in ("wind", "blast", "stagger") for d in ("fromLeft", "fromRight")})
    def test_error_diffusion_branch(self): self.assertIn("errorDiffusion", {row[0] for row in fixture_values("dither", ("type",))})
    def test_directional_blur_nondefault_parameters(self): self.assertTrue(any("angle: 37" in source and "distance: 75" in source for source in effect_sources("directionalBlur")))
    def test_plastic_wrap_nondefault_light_direction(self): self.assertTrue(any("lightDirection: vec3(" in source for source in effect_sources("plasticWrap")))
    def test_spin_blur_off_center(self): self.assertTrue(any("centerX: 0.35" in source and "centerY: 0.3" in source for source in effect_sources("spinBlur")))
